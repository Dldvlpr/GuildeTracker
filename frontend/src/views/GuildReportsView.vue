  ...
  <section class="mx-auto max-w-6xl px-4 py-8">
    ...
    <div v-else>
      <header class="flex items-center justify-between mb-8">
        <div>
          ...
          <p class="text-slate-300">
            Suivre l'activité et la progression de {{ guild?.name }}
          </p>
        </div>
        <RouterLink
          :to="`/guild/${guildId}`"
      ...
      <div class="rounded-2xl border border-white/10 bg-white/5 p-8 text-center">
        ...
        <p class="text-slate-300">
          Les rapports de guilde seront bientôt disponibles pour suivre l'activité et la progression.
        </p>
      </div>
    </div>
  </section>
...
defineOptions({ name: 'GuildReportsView' })
