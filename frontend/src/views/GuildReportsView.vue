  ...
  <section class="mx-auto max-w-6xl px-4 py-8">
    ...
    <div v-else>
      <header class="flex items-center justify-between mb-8">
        <div>
          <h1 class="text-2xl md:text-3xl font-bold text-white mb-2">📊 Guild Reports</h1>
          <p class="text-slate-300">Track activity and progression for {{ guild?.name }}</p>
        </div>
        <RouterLink
          :to="`/guild/${guildId}`"
      ...
      <div class="rounded-2xl border border-white/10 bg-white/5 p-8 text-center">
        ...
        <p class="text-slate-300">Guild reports will be available soon.</p>
      </div>
    </div>
  </section>
</template>
