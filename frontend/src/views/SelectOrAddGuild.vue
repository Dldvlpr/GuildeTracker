<template>
  <section class="mx-auto max-w-6xl px-4 py-8">
    ...
    >
        ...
        >
          ...
          >
            <div class="flex items-center gap-3">
              <div
                class="relative h-11 w-11 shrink-0 rounded-xl grid place-items-center text-base font-semibold
                       bg-white/5 ring-1 ring-inset ring-white/10 group-hover:bg-white/10"
                :aria-label="(g.name) + ' logo'"
              >
                {{ initials(g.name) }}
                <div class="pointer-events-none absolute -inset-0.5 rounded-[inherit] opacity-0 group-hover:opacity-100 transition
                            bg-[radial-gradient(18rem_18rem_at_30%_-20%,rgba(99,102,241,0.35),transparent_60%)]"></div>
              </div>
              ...
              <div class="min-w-0 flex-1">
                ...
                <div class="mt-1 flex items-center gap-2">
                  <span
                    class="inline-flex items-center gap-1 rounded-lg px-2 py-0.5 text-[11px] leading-5
                           bg-white/5 ring-1 ring-inset ring-white/10 text-slate-300"
                    v-if="g.faction"
                  >
                    🏰 {{ g.faction }}
                  </span>
                  <span
                    v-else
                    class="inline-flex items-center gap-1 rounded-lg px-2 py-0.5 text-[11px] leading-5
                           bg-white/5 ring-1 ring-inset ring-white/10 text-slate-400"
                  >
                    🏷️ Guilde
                  </span>
                </div>
              ...
              <div
                class="ml-auto h-8 w-8 grid place-items-center rounded-lg ring-1 ring-inset ring-white/10
            ...
            <div class="mt-4 border-t border-white/10 pt-3">
              <div class="grid grid-cols-3 gap-2 text-center text-xs text-slate-400">
                <div class="rounded-lg bg-white/5 py-2 ring-1 ring-inset ring-white/10 group-hover:bg-white/10">
      ...
      <button
        :class="[
          'block w-full max-w-[280px] rounded-2xl p-[1px] transition',
          open
            ? 'bg-gradient-to-br from-indigo-500/40 via-fuchsia-500/30 to-cyan-400/30 shadow-[0_0_0_1px_rgba(255,255,255,0.08)]'
            : 'bg-gradient-to-br from-white/10 via-white/5 to-transparent hover:from-indigo-500/20 hover:via-fuchsia-500/10 hover:to-transparent'
        ]"
