  ...
  <section class="mx-auto max-w-6xl px-4 py-8">
    ...
    <div v-else>
      <header class="flex items-center justify-between mb-8">
        <div>
          ...
          <p class="text-slate-300">
            Planifier et organiser vos événements de guilde pour {{ guild?.name }}
          </p>
        </div>
        <RouterLink
          :to="`/guild/${guildId}`"
      ...
      <div class="rounded-2xl border border-white/10 bg-white/5 p-8 text-center">
        ...
        <p class="text-slate-300">
          Le calendrier des raids sera bientôt disponible pour planifier vos événements.
        </p>
      </div>
    </div>
  </section>
...
defineOptions({ name: 'RaidCalendarView' })
