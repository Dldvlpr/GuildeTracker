  ...
  <section class="mx-auto max-w-6xl px-4 py-8">
    ...
    <div v-else>
      <header class="flex items-center justify-between mb-8">
        <div>
          ...
          <p class="text-slate-300">
            Analyser participation, performance et loot de {{ guild?.name }}
          </p>
        </div>
        <RouterLink
          :to="`/guild/${guildId}`"
      ...
      <div class="rounded-2xl border border-white/10 bg-white/5 p-8 text-center">
        ...
        <p class="text-slate-300">
          Les statistiques de raid seront bientôt disponibles pour analyser les performances.
        </p>
      </div>
    </div>
  </section>
...
defineOptions({ name: 'RaidStatsView' })
