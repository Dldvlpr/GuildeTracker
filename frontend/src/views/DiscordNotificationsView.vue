  ...
  <section class="mx-auto max-w-6xl px-4 py-8">
    ...
    <div v-else>
      <header class="flex items-center justify-between mb-8">
        <div>
          ...
          <p class="text-slate-300">
            Webhooks pour les événements importants de {{ guild?.name }}
          </p>
        </div>
        <RouterLink
          :to="`/guild/${guildId}`"
      ...
      <div class="rounded-2xl border border-white/10 bg-white/5 p-8 text-center">
        ...
        <h2 class="text-xl font-semibold text-white mb-2">Fonctionnalité en développement</h2>
        <p class="text-slate-300">
          Les notifications Discord seront bientôt disponibles pour vos événements importants.
        </p>
      </div>
    </div>
  </section>
